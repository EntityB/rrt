...
function callEET(data, privateKey, publicKey, password, cb) {
    soap.createClient(url, function (err, client) {
        ...
        client.EETService.EETServiceSOAP.OdeslaniTrzby(data, function (err, result, raw, soapHeader) {
            if (err) {
                cb(err);
                return;
            }
            cb(null, result, raw, soapHeader);
        });
    });
}
...
module.exports = {
    eetTest: function (cb) {
        var password = 'eet';
        var privateKey = fs.readFileSync(__dirname + "/data-test/private.key");
        var publicKey = fs.readFileSync(__dirname + "/data-test/public.key");
        callEET(fakeData, privateKey, publicKey, password, cb);
    },
    eet: function (data, cb) {
        callEET(data, privateKey, publicKey, password, cb);
    },
    setPrivateKey: function (filePath) {
        privateKey = fs.readFileSync(filePath);
    },
    setPublicKey: function (filePath) {
        publicKey = fs.readFileSync(filePath);
    },
